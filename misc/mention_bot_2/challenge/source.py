...
for him to reach you out!
    ...
    /auth user pass - authenticate to access sensitive information
"""
...
async def auth_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # check if we're not int private chat
    if update.message.chat.type != "private":
        await update.message.reply_text("can only be performed in a private chat!")
        return
    if len(ctx.args) != 2:
        await update.message.reply_text("provide username and password")
        return
    username = ctx.args[0]
    password = ctx.args[1]
    cursor = conn.cursor()
    query = f'SELECT * FROM private_users WHERE username = "{username}" AND password = "{password}"'
    print(f"query: {query}")
    cursor.execute(query)
    result = cursor.fetchone()
    if result:
        await update.message.reply_text(f"you thought the flag was here, but it was me, kono Dio Daaa!")
    else:
...
BOT_TOKEN = "7162298180:AAEw-7uF9GLafAvZthGkoKeQm9DkJOCCOIk"
